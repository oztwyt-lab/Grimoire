...
export default function CreateRecipe() {
  ...
  const handleSave = async () => {
    ...
    try {
      await addDoc(collection(db, 'recipes'), {
        userId: user?.uid, name, steps, ingredients, createdAt: serverTimestamp(),
      });
  ...
  return (
    <ScrollView style={crStyles.container} contentContainerStyle={crStyles.content}>
      ...
      <TextInput style={crStyles.input} placeholder={t('create_name_placeholder')} placeholderTextColor="#4a4a6a" value={name} onChangeText={setName} />
      <Text style={crStyles.label}>{t('create_ingredients_label')}</Text>
      <View style={crStyles.ingredientRow}>
        ...
        <Pressable style={({ pressed }) => [crStyles.addTile, pressed && { backgroundColor: '#2d2d4e' }]} onPress={() => router.push('/ingredient-picker')}>
          <Text style={crStyles.addTileText}>+</Text>
        </Pressable>
      </View>
      <Text style={crStyles.label}>{t('create_steps_label')}</Text>
      <TextInput style={[crStyles.input, crStyles.textArea]} placeholder={t('create_steps_placeholder')} placeholderTextColor="#4a4a6a" value={steps} onChangeText={setSteps} multiline />
      <Pressable style={({ pressed }) => [crStyles.button, pressed && { backgroundColor: '#2d2d4e' }]} onPress={handleSave} disabled={saving}>
